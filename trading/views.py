...
def paper_pnl_snapshot(account: PaperAccount) -> dict:
    """Return realized, unrealized and total mark-to-market paper P/L."""
    realized = (
        Trade.objects.filter(mode="paper", status="closed")
...
def scanner_view(request):
    if request.method == "POST":
        try:
            ...
            messages.success(request, "Market scan completed.")
...
def paper_view(request):
    ...
    if request.method == "POST":
        paper_cycle()
        messages.success(request, "Paper engine cycle completed.")
    ...
    return render(
        ...
        "paper.html",
        {
            ...
            "closed_trades": Trade.objects.filter(mode="paper", status="closed")[:100],
        },
    )
...
def backtest_view(request):
    if request.method == "POST":
        ...
        try:
            run_backtest(symbol)
            messages.success(request, f"Backtest complete for {symbol}.")
        except Exception as e:
...
def live_view(request):
    ...
    return render(
        ...
        {
            "gate": gate,
            "live_trades": Trade.objects.filter(mode__in=["testnet", "live"])[:50],
        },
    )
    ...
    return JsonResponse(
        {
            ...
            "live_gate": bool(gate and gate.eligible),
